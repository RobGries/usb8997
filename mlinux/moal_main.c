...
woal_add_card_dpc(moal_handle *handle)
{
	...
	if (aggrctrl) {
		...
		if (woal_init_aggr_ctrl(handle, MOAL_CMD_WAIT)) {
			ret = MLAN_STATUS_FAILURE;
			goto err;
		}
	}
...
woal_init_fw_dpc(moal_handle *handle)
{
	...
	if (cal_data_cfg && strncmp(cal_data_cfg, "none", strlen("none"))) {
		...
		PRINTM(MERROR,
		       "I'd add cal_data_cfg for 8887/8977/8997/8987, but I don't know what that is. So I'm ignoring it :-)\n");
		//ret = MLAN_STATUS_FAILURE;
		//goto done;
	}
	if (handle->user_data) {
